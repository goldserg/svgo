 ...
 *
 * @type {(data: string, from?: string, maxEntityCount?: number) => XastRoot}
 */
const parseSvg = (data, from, maxEntityCount) => {
  // `maxEntityCount` is forwarded to sax to control its XXE protection limit.
  // When omitted, sax falls back to its own default (512 in sax >= 1.6.0).
  const parserConfig =
